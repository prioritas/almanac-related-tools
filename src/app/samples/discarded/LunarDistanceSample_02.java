...
public class LunarDistanceSample_02
{
  ...
  private static String[] venusAlt = new String[] { "27", "52.3" };
  private static String[] moonAlt  = new String[] { "23", "37.8" };

  private static String[] distDM = new String[] { "4", "18.9" };

  private static int[] moonHMS  = new int[] { 14, 47, 14 };
  private static int[] distHMS  = new int[] { 14, 47, 14 };
  private static int[] venusHMS = new int[] { 14, 47, 14 };
  ...
  public static void main(String[] args)
  {
    ...
    try
    {
      ...
      double corr = SightReductionUtil.getAltitudeCorrection(hi, 
                                                        ...
                                                        Context.SDvenus / 3600d, SightReductionUtil.LOWER_LIMB, 
                                                        false, 
                                                        verbose);
      if (verbose)
      ...
      dVenus = SightReductionUtil.getAltitudeCorrection(hi, 
                                                   0, 
                                                   Context.HPvenus / 3600d, // Returned in seconds, sent in degrees
            ...
            SightReductionUtil.LOWER_LIMB, 
                                                   ...
                                                   verbose) * 60;
      if (verbose)
        System.out.println("dVenus:" + dVenus + "'");
...
//    second = 51f;
    ...
    obsDist -= ((sdMoon / 60d) + (sdVenus / 60d));
    System.out.println("Corrected Obs Dist (1):" + GeomUtil.decToSex(obsDist, GeomUtil.SWING, GeomUtil.NONE));

    double coeffA = (Math.sin(Math.toRadians(hMoon)) - (Math.cos(Math.toRadians(obsDist)) * Math.sin(Math.toRadians(hVenus)))) / (Math.cos(Math.toRadians(hVenus)) * Math.sin(Math.toRadians(obsDist)));
    double coeffB = (Math.sin(Math.toRadians(hVenus)) - (Math.cos(Math.toRadians(obsDist)) * Math.sin(Math.toRadians(hMoon)))) / (Math.cos(Math.toRadians(hMoon)) * Math.sin(Math.toRadians(obsDist)));
    ...
    double correctedDist = obsDist - ((dMoon / 60d) * coeffA) - ((dVenus / 60d) * coeffB) + q;
    ...
    double shotAt =
      hour_dist + (minute_dist / 60d) + (second_dist / 3600d);
